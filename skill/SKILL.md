---
name: skilldeck
description: Generates professional slide decks. Mixed pipeline — narrative slides via AI image generation, data/chart slides via SVG templates — exporting a single editable PPTX. Use when user asks to "create slides", "make a presentation", "generate deck", "slide deck", or "PPT".
version: 2.0.0
---

# Slide Deck Generator

Transform content into professional slide deck images. The deck is designed for **reading and sharing** (self-explanatory slides, logical scroll flow, social-media-friendly) rather than live presentation — that assumption drives every layout and density decision below. Exported **PowerPoint is editable** (native text boxes via MinerU layout + skilldeck `editable_pptx`); see `references/editable-pptx.md` and repository `.env.example` for `MINERU_TOKEN`.
...
| Script | Purpose |
|--------|---------|
| `python -m deck_assembler` | **Mixed-deck assembly** (image + chart slides → one editable PPTX). Run from skilldeck repo root. Image slides require `MINERU_TOKEN`; chart slides do not. |
| `python -m editable_pptx` | Image-only editable PPTX (legacy entry; equivalent to `deck_assembler` when no chart slides exist). See `references/editable-pptx.md`. |
| `python skill/scripts/render_chart_slide.py <spec.json> -o out.svg` | Render a single CHART_SPEC JSON to a chart-slide SVG (offline, no API). |
| `python skill/scripts/svg_quality_checker.py <dir>` | Lint generated SVGs against the technical ban-list (from ppt-master). |
| `scripts/merge-to-pdf.ts` | Merge slides into PDF (read-only share) |
| `scripts/merge-to-pptx.ts` | **Deprecated** — flat raster slides only; do not use for editable decks |

## Slide Kinds

The pipeline supports two slide kinds inside a single deck:

| Kind | Trigger | Path |
|---|---|---|
| **Image slide** (default) | `**Filename**: NN-slide-{slug}.png` | LLM image prompt → image API → PNG → MinerU layout → editable PPTX shapes |
| **Chart slide** | `**Render**: chart` line + `<CHART_SPEC>` JSON block + `**Filename**: NN-slide-{slug}.svg` | CHART_SPEC → templated SVG (`render_chart_slide.py`) → svg_to_pptx → native DrawingML |

Choose chart slides for any quantitative content (KPIs, bar/line/pie/donut/area, tables, comparisons). Choose image slides for narrative, hero illustrations, covers, and metaphor. The 8 supported chart templates and full `<CHART_SPEC>` schema are documented in `references/outline-template.md`.
...
| `--slides <N>` | Target slide count (min 1, max 30; 8-25 typical for long docs) |
...
| Path | Scope |
|------|-------|
| `.skilldeck/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/skilldeck/EXTEND.md` | XDG |
| `$HOME/.skilldeck/EXTEND.md` | User home |
...
**PPTX (editable, mixed image + chart)** — from the **skilldeck** project root, with `MINERU_TOKEN` in `.env` (only required when image slides are present):

```bash
python -m deck_assembler <slide-deck-dir> [--output path/to/deck.pptx]
```

**PDF** (optional, flat pages):
