...
Structure for user preferences in `.skilldeck/EXTEND.md`.
...
## Editable PPTX (skilldeck + MinerU)

When using **skilldeck** (`python -m editable_pptx`), layout comes from **MinerU only**. Configure tokens in the skilldeck repo `.env` (not EXTEND.md). See skilldeck `.env.example` and `references/editable-pptx.md`.

| Variable | Role |
|----------|------|
| `MINERU_TOKEN` | Required for editable `.pptx` |
| `MINERU_API_BASE` | Optional; default `https://mineru.net` |
| `MINERU_MODEL_VERSION` | Optional; `vlm` or `pipeline` |
| `EDITABLE_PPTX_BG_MODE` | Optional; `edge` (default), `whiteout`, or `none` |
...
Then use with: `/skilldeck content.md --style style-name`
...
1. `.skilldeck/EXTEND.md` (project)
2. `$HOME/.skilldeck/EXTEND.md` (user)
